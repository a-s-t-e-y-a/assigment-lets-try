...
app.use(statusMonitor({
  title: 'Delivery Service Monitor',
  path: '/status',
  healthChecks: [{
    protocol: 'http',
    host: 'localhost',
...
const startConsumingOrders = async () => {
  await consumeMessages(rabbitMQConfig.queues.deliveryOrders, async (message) => {
    console.log('Received message:', message);
    
    if (message.eventType === 'ORDER_PREPARED') {
      await DeliveryService.handleOrderPrepared(message);
    }
  });
};
